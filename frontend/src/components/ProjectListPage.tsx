...
import { listProjects, createProject, listTemplates, deleteProject } from "@/lib/api";
            ...
            <div className="space-y-2">
              {projects.map((p) => (
                <div
                  ...
                  className="w-full text-left p-4 rounded-xl border border-border bg-surface hover:border-accent/30 hover:shadow-xs transition-all group cursor-pointer"
                >
                  <div className="flex items-start justify-between">
                    ...
                    <div className="flex items-center gap-1">
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          if (confirm(`Delete project "${p.display_name || p.name}"? This cannot be undone.`)) {
                            deleteProject(p.name).then(() => loadProjects()).catch(() => {});
                          }
                        }}
                        className="p-1 rounded-md hover:bg-red-50 dark:hover:bg-red-900/20 text-text-muted hover:text-red-500 opacity-0 group-hover:opacity-100 transition-all"
