...
- **URL:** https://github.com/agentscope-ai/agentscope/tree/main/examples/workflows/multiagent_concurrent
...
1. **asyncio.gather** - 原生Python并发方式
2. **fanout_pipeline** - AgentScope提供的并发管道，自动收集结果
3. **自定义Agent** - 继承AgentBase实现reply方法
4. **metadata** - 可附加自定义数据到消息
5. **enable_gather=True** - 自动收集所有agent的结果
